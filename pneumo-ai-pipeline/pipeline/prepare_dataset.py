...
def prepare_training_data(config_path):
    ...
    download_from_s3 = steps.get('download_from_s3', False)
    segment_lungs = steps.get('segment_lungs', False)
    preprocess_images = steps.get('preprocess_images', False)
    upload_to_s3 = steps.get('upload_to_s3', False)

    # 디렉토리 경로 설정
    data_dir = config['directories']['data_dir']
    input_dir = os.path.join(data_dir, config['directories']['input_dir'])
    segmented_dir = os.path.join(data_dir, config['directories']['segmented_dir'])
    preprocessed_dir = os.path.join(data_dir, config['directories']['preprocessed_dir'])
    ...
    os.makedirs(os.path.join(segmented_dir), exist_ok=True)
    os.makedirs(os.path.join(preprocessed_dir), exist_ok=True)

    # 1. S3에서 이미지 파일 다운로드 (설정에 따라 실행)
    if download_from_s3 and bucket_name and s3_input_prefix:
        ...
        try:
            # S3에서 첫 번째 레벨 폴더 목록 조회 (train, val, test 등)
            ...
            # 첫 번째 레벨 폴더 구조 확인 (train, val, test)
            dataset_prefixes = []
            for page in pages:
                if 'CommonPrefixes' in page:
                    for prefix in page['CommonPrefixes']:
                        dataset_prefixes.append(prefix['Prefix'])
            
            # 데이터셋 폴더가 없는 경우 전체 다운로드
            if not dataset_prefixes:
                downloaded_files = s3.download_directory(full_s3_prefix, os.path.join(data_dir))
                print(f"S3에서 다운로드 완료: {len(downloaded_files)}개 파일")
            else:
                # 데이터셋별로 다운로드 (train, val, test)
                total_downloaded = 0
                
                for dataset_prefix in dataset_prefixes:
                    dataset_name = os.path.basename(dataset_prefix.rstrip('/'))
                    dataset_dir = os.path.join(data_dir, dataset_name)
                    os.makedirs(dataset_dir, exist_ok=True)
                    
                    # 해당 데이터셋 내의 클래스 폴더 조회 (0, 1, 2)
                    class_pages = paginator.paginate(
                        Bucket=bucket_name,
                    ...
                    for class_page in class_pages:
                        if 'CommonPrefixes' in class_page:
                            for class_prefix in class_page['CommonPrefixes']:
                                class_prefixes.append(class_prefix['Prefix'])
                    
                    # 클래스 폴더가 없는 경우 데이터셋 전체 다운로드
                    if not class_prefixes:
                        downloaded_files = s3.download_directory(dataset_prefix, dataset_dir)
                        total_downloaded += len(downloaded_files)
                        print(f"데이터셋 {dataset_name} 다운로드 완료: {len(downloaded_files)}개 파일")
                    else:
                        # 클래스별로 다운로드 (0, 1, 2)
                        ...
                        for class_prefix in class_prefixes:
                            ...
                            dataset_downloaded += len(downloaded_files)
                            print(f"데이터셋 {dataset_name}, {class_name} 다운로드 완료: {len(downloaded_files)}개 파일")
                        
                        total_downloaded += dataset_downloaded
                        print(f"데이터셋 {dataset_name} 다운로드 완료: {dataset_downloaded}개 파일")
                
                print(f"S3에서 다운로드 완료: 총 {total_downloaded}개 파일")
        except Exception as e:
            print(f"S3 다운로드 중 오류 발생: {str(e)}")
    else:
        print("S3 다운로드 단계 건너뛰기")
    ...
    if preprocess_images:
        ...
        for root, _, files in os.walk(source_dir):
            for file in files:
                if file.lower().endswith(('.png', '.jpg', '.jpeg')):
                    preprocessed_paths.append(os.path.join(root, file))

        total_images = len(preprocessed_paths)
        print(f"전처리 시작: 총 {total_images}개 이미지")

        # 클래스별 구조를 유지하면서 이미지 전처리
        for i, img_path in enumerate(preprocessed_paths, 1):
            try:
                # 진행 상황 출력
                if i % 10 == 0 or i == total_images:
                    print(f"전처리 진행 중: {i}/{total_images} ({i/total_images*100:.1f}%)")
                
                # 원본 경로에서 상대 경로 계산 - 수정된 부분
