...
window.config = {
  ...
  callbacks: {
    /**
     * Called when a DICOM study is successfully uploaded.
     * @param {string} studyInstanceUID
     */
    async onStudyStored(studyInstanceUID) {
      console.log('📦 Study stored:', studyInstanceUID);

      try {
        const res = await fetch('http://orchestrator_dev:8000/api/webhook/on-store', {
          method: 'POST',
        ...
        console.log('✅ Webhook sent successfully:', text);
      } catch (err) {
