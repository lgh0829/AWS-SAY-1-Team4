...
def main():
    ...
    class_sample_count = [class_count[i] for i in range(len(class_count))]

    # 각 클래스의 가중치 = 1 / 해당 클래스 개수
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, sampler=sampler, num_workers=num_workers, pin_memory=device.type=='cuda')
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size, num_workers=num_workers, pin_memory=device.type=='cuda')
    test_loader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=num_workers, pin_memory=device.type=='cuda')
    ...
    # MLflow 사용 여부 확인
    use_mlflow = bool(os.environ.get('MLFLOW_TRACKING_ARN')) and bool(os.environ.get('MLFLOW_EXPERIMENT_NAME'))
    ...
    if use_mlflow:
        ...
        # MLflow 사용하지 않을 때 기본 학습 루프
        print("MLflow 비활성화 상태입니다.")
