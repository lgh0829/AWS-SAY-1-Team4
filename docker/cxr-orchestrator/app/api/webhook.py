...
from app.model.schema import WebhookPayload, WebhookResponse
...
async def on_store_webhook(payload: WebhookPayload):
    try:
        ...
        # 여기에 파이프라인 로직 추가 예정
        # pipeline_result = await pipeline.run_inference_pipeline(study_uid, output_dir)
        # logging.info(f"Pipeline result: {pipeline_result}")
        # output 디렉토리 정리
        # for file in output_dir.glob("*"):
        #     file.unlink()
        return {"message": f"Webhook received for study {study_uid}"}
